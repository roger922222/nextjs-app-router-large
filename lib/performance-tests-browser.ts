...
// Browser-compatible Performance Test Runner
export class PerformanceTestRunner {
  ...
  async runTest(test: TestCase): Promise<TestResult> {
    ...
    try {
      ...
      const memoryAfter = performance.memory ? (performance as any).memory.usedJSHeapSize : 0;
...
export function generateTestSummary(reports: TestReport[]): {
  ...
  return {
    ...
    totalWarnings,
    averageDuration,
    recommendations: recommendations.length > 0 ? recommendations : ['所有性能测试均已通过！系统运行良好。']
  };
}